...
`load()` and `save()` have been restructured and replaced with different functions. `read()` and `write()` are now used for reading and writing images.
...
However, these functions are only implemented for Node.js. To read an image in browser via URL, you can use `fetchURL`.
Or, if you have an HTML element that you want to read an image from, use `readImg`.
